...
class LinearRegression(analysis.Analysis):
    ...
    def linear_regression(self, ind_vars, dep_var):
        ...
        A_new = np.hstack([np.ones([self.A.shape[0], 1]), self.A])

        c, _, _, _ = scipy.linalg.lstsq(A_new, self.y)

        self.intercept = c[0].squeeze()  
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        x_vals, y_vals = analysis.Analysis.scatter(self, ind_var=ind_var, dep_var=dep_var, title=title)

        x_min, x_max = np.min(x_vals), np.max(x_vals)
        x_range = np.linspace(x_min, x_max, 100).reshape(-1, 1)
        
        if self.p > 1:
            X_poly = self.make_polynomial_matrix(x_range, self.p)
            y_range = X_poly @ self.slope + self.intercept
        else:
            y_range = self.intercept + self.slope * x_range
        
        plt.plot(x_range, y_range, color='red')

        if self.R2 is not None:
            title += f"  R^2 = {self.R2:.2f}"
        plt.title(title)
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), hists_on_diag=True):
        ...
        '''
        fig, axes = analysis.Analysis.pair_plot(self, data_vars, title='')
        ...
        for i in range(num_vars):
            for j in range(num_vars):
                if hists_on_diag and i == j:
                    axes[i, j].clear()
                    axes[i, j].hist(self.data.select_data([data_vars[i]]).flatten(), bins=10, color='gray')
                    axes[i, j].set_title(f'{data_vars[i]} Distribution')
                else:
                    self.linear_regression([data_vars[j]], data_vars[i])
                    
                    x_data = self.data.select_data([data_vars[j]]).flatten()
                    y_data = self.data.select_data([data_vars[i]]).flatten()
                    axes[i, j].scatter(x_data, y_data, color='blue')

                    x_vals = np.linspace(np.min(x_data), np.max(x_data), 100).reshape(-1, 1)
                    y_pred = self.predict(x_vals)
                    axes[i, j].plot(x_vals, y_pred, color='red')

                    r2 = self.r_squared(self.predict())
                    axes[i, j].set_title(f'R²: {r2:.2f}')
    ...
    def make_polynomial_matrix(self, A, p):
        ...
        '''
        X_poly = np.hstack([A**i for i in range(1, p + 1)])
        return X_poly
    ...
    def poly_regression(self, ind_var, dep_var, p):
        ...
        polyA = self.make_polynomial_matrix(ind, p)

        ones_column = np.ones((ind.shape[0], 1))
        self.A = np.hstack((ones_column, polyA))

        c, residuals, rank, s = scipy.linalg.lstsq(self.A, self.y)
