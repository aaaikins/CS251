...
class LinearRegression(analysis.Analysis):
    ...
    def linear_regression(self, ind_vars, dep_var):
        ...
        coefficients, residues, _, _ = scipy.linalg.lstsq(A, Y)
        
        SSE = residues if len(residues) > 0 else np.sum((Y - np.dot(A, coefficients))**2)
        
        mean_y = np.mean(Y)
        smd = np.sum((Y - mean_y) ** 2)
        R_squared = 1 - (SSE / smd)
        
        self.intercept = coefficients[0]
        self.slope = coefficients[1:]
        self.residuals = Y - np.dot(A, coefficients)
        self.R2 = R_squared
