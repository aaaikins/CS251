...
class CatData(data.Data):
    ...
    def get_cat_levels_int(self, header):
        ...
        '''
        str_levels = self.cats2levels[header]
        return np.arange(len(str_levels))
    ...
    def int2strlevels(self, header, int_levels):
        ...
        '''
        all_str_levels = self.cats2levels[header]
        return [all_str_levels[i] for i in int_levels]
    ...
    def str2intlevels(self, header, str_levels):
        ...
        '''
        str_to_int = []
        for item in str_levels:
            str_to_int.append(self.cats2levels[header].index(item))
        return str_to_int
    ...
    def filter(self, header, strlevel):
        ...
        '''
        int_level = self.str2intlevels(header, [strlevel])[0]
        
        col_index = self.header2col[header]
        
        self.data = self.data[np.squeeze(self.data[:, col_index]) == int_level]
