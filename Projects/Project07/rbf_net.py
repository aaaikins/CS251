...
class RBF_Net:
    ...
    def avg_cluster_dist(self, data, centroids, cluster_assignments, kmeans_obj):
        ...
        '''
        avg_dist = np.zeros((centroids.shape[0],))
        for i in range(centroids.shape[0]):
            group = data[cluster_assignments==i, :]
            dists_from_center = kmeans_obj.dist_pt_to_centroids(centroids[i], group)
            avg_dist[i] = dists_from_center.mean()
        return avg_dist
    ...
    def hidden_act(self, data):
        ...
        for i in range(self.k):
            numerator = kmeans_obj.dist_pt_to_centroids(pt=self.prototypes[i], centroids=data)**2
            denominator = 2 * (self.sigmas[i])**2 + (10**-8)
            hidden[:, i] = np.exp(-(numerator)/(denominator))
        return hidden
    ...
    def output_act(self, hidden_acts):
        ...
        '''
        hidden = np.hstack((hidden_acts, np.ones((hidden_acts.shape[0],1))))
        return (hidden @ self.wts)
    ...
    def train(self, data, y):
        ...
        for i in range(self.num_classes):
            z = y==i
            self.wts[:, i] = self.linear_regression(hidden_layer, z)
