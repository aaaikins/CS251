...
class PCA:
    ...
    def fit(self, vars, normalize_dataset=False):
        ...
        sorted_indices = np.argsort(self.e_vals)[::-1]
        self.e_vals = self.e_vals[sorted_indices]
        self.e_vecs = self.e_vecs[:, sorted_indices]
    ...
    def elbow_plot(self, num_pcs_to_keep=None):
        ...
        if self.cum_var is None:
            raise ValueError('Cannot plot cumulative variance. Compute PCA first.')

        total_pcs = len(self.cum_var)
        
        if num_pcs_to_keep is None:
            num_pcs_to_keep = total_pcs
        elif num_pcs_to_keep > total_pcs:
            raise ValueError(f'Cannot plot {num_pcs_to_keep} PCs. Only {total_pcs} PCs available.')
        

        
        x_values = np.arange(1, num_pcs_to_keep + 1)

        y_values = self.cum_var[:num_pcs_to_keep]

        plt.figure(figsize=(10, 6))
        plt.plot(x_values, y_values, marker='o', markersize=8, linestyle='-')

        plt.title('Elbow Plot of Cumulative Variance', fontsize=16)
    ...
    def pca_project(self, pcs_to_keep):
        ...
        if self.e_vecs is None:
            raise ValueError("Must fit PCA before projecting")
        if not all(isinstance(pc, (int, np.integer)) and 0 <= pc < self.e_vecs.shape[1] for pc in pcs_to_keep):
            raise ValueError("Invalid PC indices")
            
        A_numpy = self.A if isinstance(self.A, np.ndarray) else self.A.to_numpy()
        selected_evecs = self.e_vecs[:, pcs_to_keep]
        
        self.A_proj = np.dot(A_numpy, selected_evecs)
        return self.A_proj 
    ...
    def pca_then_project_back(self, top_k):
        ...
        '''
        if self.e_vecs is None or self.e_vals is None:
            raise ValueError("PCA must be fitted before back-projection.")

        selected_evecs = self.e_vecs[:, :top_k]
        A_proj_top_k = self.A @ selected_evecs

        A_reconstructed = A_proj_top_k @ selected_evecs.T

        if self.normalized:
            A_reconstructed = A_reconstructed * (self.orig_maxs - self.orig_mins) + self.orig_mins
    ...
    def loading_plot(self):
        ...
        '''
        if self.e_vecs is None:
            raise ValueError("PCA must be fitted before creating a loading plot.")

        top_2_evecs = self.e_vecs[:, :2]

        plt.figure(figsize=(8, 8))
        for i in range(top_2_evecs.shape[0]):
            plt.plot([0, top_2_evecs[i, 0]], [0, top_2_evecs[i, 1]], 'b-', lw=1.5)
            plt.annotate(self.vars[i], (top_2_evecs[i, 0], top_2_evecs[i, 1]), fontsize=12, color='red')

        plt.xlabel('First Principal Component', fontsize=14)
        plt.ylabel('Second Principal Component', fontsize=14)
        plt.title('Loading Plot for Top 2 Principal Components', fontsize=16)
